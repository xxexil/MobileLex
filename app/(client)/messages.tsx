...
export default function MessagesScreen() {
  ...
  const params = useLocalSearchParams<{ conversationId?: string; resetThreadAt?: string; openGroups?: string; openCreateGroup?: string; fromNotification?: string; draftMessage?: string; draftSendNow?: string; draftSendToken?: string }>();
  const { triggerClientUnreadRefresh } = useNotifications();
  ...
  const handledDraftMessageRef = useRef<string | null>(null);
  const handledDraftSendTokenRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    navigation.setOptions({
      tabBarStyle: isConversationOpen
        ? { ...CLIENT_TAB_BAR_STYLE, display: 'none' }
        : CLIENT_TAB_BAR_STYLE,
    });

    return () => {
      tabBarHiddenRef.current = null;
      navigation.setOptions({ tabBarStyle: CLIENT_TAB_BAR_STYLE });
    };
  ...
  const scheduleConversationSync = useCallback((conversationId: number, forceScroll = false) => {
    ...
    const canAutoScroll = !isUserScrollingRef.current && !isAutoScrollLocked() && (forceScroll || shouldStickToBottomRef.current);
    if (canAutoScroll) {
      prioritizeThreadLatest(false);
    }
  ...
  const appendIncomingMessage = useCallback((incoming: Message, forceScroll = false) => {
    ...
    const canAutoScroll = !isUserScrollingRef.current && !isAutoScrollLocked() && (forceScroll || shouldStickToBottomRef.current);
    if (canAutoScroll) {
      scheduleConversationSync(conversationId, forceScroll);
    } else {
      if (!flushScheduledRef.current) {
        flushScheduledRef.current = true;
        requestAnimationFrame(() => flushMessageState());
  ...
  }, [bumpConversationToTop, flushMessageState, isAutoScrollLocked, scheduleConversationSync]);
  ...
  const updateOptimisticMessage = useCallback((localId: string, nextMessage: Message, forceScroll = true) => {
    const conversationId = activeConversationIdRef.current;
    if (!conversationId) return;
    const cached = fullMessagesByConversationRef.current[conversationId] ?? [];
    fullMessagesByConversationRef.current[conversationId] = cached.map((message) =>
      message.local_id === localId
        ? { ...nextMessage, local_id: localId, delivery_state: 'sent' }
        : message
    );
    flushScheduledRef.current = false;
    requestAnimationFrame(() => flushMessageState());
  ...
  const markOptimisticFailed = useCallback((localId: string) => {
    ...
    scheduleConversationSync(conversationId, true);
  }, [scheduleConversationSync]);

  const loadConversations = useCallback(async () => {
    try {
      setConversationLoadError(null);
      const [conversationResult, groupResult] = await Promise.allSettled([
        clientApi.conversations(),
        user?.id ? groupApi.groups(user.id) : Promise.resolve({ data: { groups: [] } }),
      ]);

      if (conversationResult.status === 'fulfilled') {
        const conversationData = conversationResult.value?.data;
        setConversations(sortConversationsByActivity(Array.isArray(conversationData) ? conversationData : []));
      } else {
        setConversations([]);
        const statusCode = conversationResult.reason?.response?.status;
        if (statusCode === 401) {
          setConversationLoadError('Session expired. Please sign in again.');
        } else {
          setConversationLoadError('Could not load conversations. Pull to refresh.');
        }
      }

      if (groupResult.status === 'fulfilled') {
        const groups = Array.isArray(groupResult.value?.data?.groups) ? groupResult.value.data.groups : [];
        const unreadTotal = groups.reduce((sum: number, group: any) => sum + Number(group?.unread ?? 0), 0);
        setGroupUnreadCount(unreadTotal);
      } else {
  ...
  }, [triggerClientUnreadRefresh, user?.id]);
  ...
  const loadContactUsers = useCallback(async () => {
    ...
    try {
      const { data } = await clientApi.lawyers({ limit: 200 });
      const lawyers = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : [];
      const normalized = lawyers
      ...
      setContactsError('Unable to load available lawyers right now. Tap retry.');
    } finally {
  ...
  const filteredContacts = useMemo(() => {
    const query = debouncedContactQuery.toLowerCase();
    const source = query.length >= CONTACT_SEARCH_MIN_CHARS
    ...
    if (query.length >= CONTACT_SEARCH_MIN_CHARS) return sorted;
    return sorted.slice(0, 24);
  }, [debouncedContactQuery, contactUsers, existingConversationByUser]);

  const groupedContacts = useMemo(() => {
    const existing: ContactUser[] = [];
    const fresh: ContactUser[] = [];

    for (const entry of filteredContacts) {
      if (existingConversationByUser.has(entry.id)) {
        existing.push(entry);
      } else {
        fresh.push(entry);
      }
  ...
  }, [existingConversationByUser, filteredContacts]);

  const inviteTarget = useMemo(() => debouncedContactQuery.trim(), [debouncedContactQuery]);
  const inviteIsEmail = useMemo(() => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(inviteTarget), [inviteTarget]);
  const invitePhone = useMemo(() => normalizePhoneCandidate(inviteTarget), [inviteTarget]);
  const hasNoContactMatch = useMemo(
    () => debouncedContactQuery.length >= CONTACT_SEARCH_MIN_CHARS
      && groupedContacts.existing.length === 0
      && groupedContacts.fresh.length === 0,
    [debouncedContactQuery.length, groupedContacts.existing.length, groupedContacts.fresh.length]
  );

  const inviteMessage = useMemo(() => {
    return 'Join me on LexConnect for secure legal consultations and messaging. Open: lexconnectmobile://';
  }, []);
  ...
  const inviteBySms = useCallback(async () => {
    if (!invitePhone) {
      Alert.alert('Invalid phone', 'Enter a valid phone number to send an SMS invite.');
      return;
    }

    const separator = Platform.OS === 'ios' ? '&' : '?';
    const url = `sms:${invitePhone}${separator}body=${encodeURIComponent(inviteMessage)}`;

    try {
      await Linking.openURL(url);
    } catch {
      Alert.alert('Invite failed', 'Unable to open SMS app right now.');
    }
  }, [inviteMessage, invitePhone]);

  const inviteByEmail = useCallback(async () => {
    if (!inviteIsEmail) {
      Alert.alert('Invalid email', 'Enter a valid email address to send an invite.');
      return;
    }

    const subject = encodeURIComponent('Join me on LexConnect');
    const body = encodeURIComponent(inviteMessage);
    const url = `mailto:${inviteTarget}?subject=${subject}&body=${body}`;

    try {
      await Linking.openURL(url);
    } catch {
      Alert.alert('Invite failed', 'Unable to open email app right now.');
    }
  }, [inviteIsEmail, inviteMessage, inviteTarget]);
  ...
  const retryFailedMessage = useCallback(async (message: Message) => {
    ...
    try {
      if (message.local_attachment_uri && message.attachment_name) {
        ...
        const { data } = await clientApi.sendMessageWithAttachment(selected.id, message.body || '', {
        ...
        const { data } = await clientApi.sendMessage(selected.id, message.body || '');
        updateOptimisticMessage(message.local_id, {
          id: data?.id ?? Date.now(),
          body: data?.body ?? message.body,
          created_at: data?.created_at ?? new Date().toISOString(),
  ...
  const deleteMessage = useCallback(async (message: Message, mode: 'me' | 'everyone') => {
    ...
    try {
      await clientApi.deleteMessage(message.id, mode);
      removeMessageLocally(message);
      loadConversations();
    } catch (error: any) {
  ...
  const loadMessages = useCallback(async (conv: Conversation) => {
    ...
    const hasPriorMessages = cached.length > 0 || !!conv.last_message;
    setConversationLoading(hasPriorMessages && cached.length === 0);
    inFlightConversationRef.current = conversationId;

    try {
      const { data } = await clientApi.messages(conv.id, { limit: 120 });
  ...
  const pollMessages = useCallback(async (conv: Conversation) => {
    ...
    try {
      ...
      const newMsgs = payload.filter((m) => !cachedIds.has(m.id));
      if (newMsgs.length > 0) {
    ...
    setConversations((current) => current.map((item) => (
      Number(item.id) === conversationId ? { ...item, unread: 0 } : item
    )));
    triggerClientUnreadRefresh();
    loadMessages(conv);
  }, [loadMessages, prioritizeThreadLatest, resetScrollGuards, triggerClientUnreadRefresh]);
  ...
  const closeConversation = useCallback(() => {
    ...
    loadConversations();
    triggerClientUnreadRefresh();
  }, [loadConversations, resetScrollGuards, triggerClientUnreadRefresh]);
  ...
  const openOrStartConversation = useCallback(async (contact: ContactUser) => {
    ...
    try {
      ...
      const alreadyExists = latestConversations.find((entry) => Number(entry?.other_user?.id) === Number(contact.id));
      if (alreadyExists) {
        setShowContactModal(false);
        setContactQuery('');
        setDebouncedContactQuery('');
        openConversation(alreadyExists);
        triggerClientUnreadRefresh();
        return;
      }

      if (contact.role && contact.role !== 'lawyer') {
        Alert.alert('Unavailable', 'You can only start new direct chats with lawyers.');
        return;
      }

      await clientApi.startConversation(contact.id);
      const refreshed = await clientApi.conversations();
      const refreshedConversations: Conversation[] = Array.isArray(refreshed?.data) ? refreshed.data : [];
      setConversations(sortConversationsByActivity(refreshedConversations));

      const target = refreshedConversations.find((entry) => Number(entry?.other_user?.id) === Number(contact.id));
      setShowContactModal(false);
      setContactQuery('');
      setDebouncedContactQuery('');
  ...
  }, [existingConversationByUser, openConversation, triggerClientUnreadRefresh]);
  ...
  useEffect(() => {
    if (params?.openCreateGroup === '1') {
      router.push({ pathname: '/(client)/group-chat', params: { openCreate: '1' } } as any);
      return;
    }

    if (params?.openGroups === '1') {
      router.push('/(client)/group-chat');
    }
  }, [params?.openCreateGroup, params?.openGroups, router]);
  ...
  useEffect(() => {
    const draft = typeof params?.draftMessage === 'string' ? params.draftMessage.trim() : '';
    if (!draft || handledDraftMessageRef.current === draft) return;

    handledDraftMessageRef.current = draft;
    setNewMsg((previous) => (previous.trim() ? previous : draft));
  }, [params?.draftMessage]);
  ...
  async function sendTextMessage(text: string, forceScroll = true) {
    if (!selected || !text.trim()) return;
    setSending(true);
    setShowEmoji(false);
    const normalizedText = text.trim();
    const canAutoScroll = forceScroll && !isUserScrollingRef.current && !isAutoScrollLocked() && shouldStickToBottomRef.current;
    const { localId, message } = createOptimisticMessage({
      body: normalizedText,
      sender_id: user?.id,
      is_mine: true,
    });
    appendIncomingMessage(message, canAutoScroll);
    try {
      const { data } = await clientApi.sendMessage(selected.id, normalizedText);
      updateOptimisticMessage(localId, {
        id: data?.id ?? Date.now(),
        body: data?.body ?? normalizedText,
        created_at: data?.created_at ?? new Date().toISOString(),
        time: data?.time,
        sender_id: data?.sender_id ?? user?.id,
        is_mine: true,
  ...
  async function sendQuickEmojiReaction(emoji: string = '👍') {
    if (!selected) return;
    autoScrollLockUntilRef.current = Date.now() + 1800;
    suppressAutoScrollRef.current = true;
    shouldStickToBottomRef.current = false;
    setShowEmoji(false);
    await sendTextMessage(emoji, false);
    requestAnimationFrame(() => {
      suppressAutoScrollRef.current = false;
    });
  }
  ...
  async function sendMessage() {
    ...
    setNewMsg('');
    await sendTextMessage(text, true);
  }

  useEffect(() => {
    const sendNow = params?.draftSendNow === '1';
    const tokenValue = typeof params?.draftSendToken === 'string' ? params.draftSendToken : '';
    const draft = typeof params?.draftMessage === 'string' ? params.draftMessage.trim() : '';

    if (!sendNow || !tokenValue || !draft || !selected || sending) return;
    if (handledDraftSendTokenRef.current === tokenValue) return;

    const routeConversationId = Number(params?.conversationId || 0);
    if (routeConversationId > 0 && routeConversationId !== Number(selected.id)) return;

    handledDraftSendTokenRef.current = tokenValue;
    setNewMsg((previous) => (previous.trim() === draft ? '' : previous));
    void sendTextMessage(draft);
  }, [params?.conversationId, params?.draftMessage, params?.draftSendNow, params?.draftSendToken, selected, sendTextMessage, sending]);
    ...
    async function sendAttachment(uri: string, name: string, mimeType: string) {
      ...
      const canAutoScroll = !isUserScrollingRef.current && !isAutoScrollLocked() && shouldStickToBottomRef.current;
      ...
      setNewMsg('');
      try {
        const { data } = await clientApi.sendMessageWithAttachment(selected.id, body, { uri, name, type: mimeType });
        updateOptimisticMessage(localId, {
          id: data?.id ?? Date.now(),
          body: data?.body ?? body,
          created_at: data?.created_at ?? new Date().toISOString(),
            ...
            <View style={styles.chatHeaderStatusRow}>
              <View
                style={[
                  ...
                  selectedPresenceStatus === 'active' && styles.chatHeaderDotActive,
                  selectedPresenceStatus === 'busy' && styles.chatHeaderDotBusy,
                  selectedPresenceStatus === 'offline' && styles.chatHeaderDotOffline,
                ]}
              />
              <Text style={styles.chatHeaderStatusText}>{selectedPresence.label}</Text>
            </View>
          </View>
        ...
        <Modal visible={threadActionsVisible} transparent animationType="fade" statusBarTranslucent onRequestClose={() => setThreadActionsVisible(false)}>
          <View style={styles.threadActionsOverlay}>
            ...
            <View style={styles.threadActionsCard}>
              ...
              <TouchableOpacity
                style={styles.threadActionsPrimaryBtn}
                onPress={() => {
                  setThreadActionsVisible(false);
                  const profileId = Number(selected?.other_user?.id ?? 0);
                  if (profileId) {
                    const returnTo = encodeURIComponent(`/(client)/messages?conversationId=${selected?.id ?? ''}`);
                    router.push({ pathname: '/lawyer/[id]', params: { id: String(profileId), returnTo } } as any);
                  }
                }}
              >
                <Icon name="person-circle-outline" size={18} color="#fff" />
                <Text style={styles.threadActionsPrimaryText}>View profile</Text>
              </TouchableOpacity>
          ...
          <View style={styles.chatStage}>
            ...
            >
              <MessengerComposer
                ...
                onSendThumb={() => void sendQuickEmojiReaction('\u{1F44D}')}
      ...
      >
        ...
        >
          <View style={styles.headerTopRow}>
            ...
            <Text style={styles.headerSub}>Talk to your lawyer directly and keep your case updates in one thread.</Text>
            <View style={styles.overviewRow}>
              ...
              <View style={styles.overviewChip}>
                <Text style={styles.overviewValue}>{groupUnreadCount}</Text>
                <Text style={styles.overviewLabel}>Groups</Text>
              </View>
            </View>
          </View>
        </AnimatedBorderCard>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            <View style={styles.groupHubCard}>
              <View style={styles.groupHubTopRow}>
                <View style={styles.groupHubIconWrap}>
                  <Icon name="people-outline" size={18} color={Colors.primary} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.groupHubTitle}>Group Chats</Text>
                  <Text style={styles.groupHubSub}>Create a group, add members, and chat like Messenger.</Text>
                </View>
                {groupUnreadCount > 0 ? (
                  <View style={styles.groupHubUnreadBadge}>
                    <Text style={styles.groupHubUnreadText}>{groupUnreadCount > 99 ? '99+' : groupUnreadCount}</Text>
                  </View>
                ) : null}
              </View>
              <View style={styles.groupHubActions}>
                <TouchableOpacity style={styles.groupHubBtn} onPress={() => router.push('/(client)/group-chat')}>
                  <Icon name="chatbubbles-outline" size={15} color="#fff" />
                  <Text style={styles.groupHubBtnText}>Open Groups</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.groupHubBtn, styles.groupHubBtnAlt]} onPress={() => router.push({ pathname: '/(client)/group-chat', params: { openCreate: '1' } } as any)}>
                  <Icon name="person-add-outline" size={15} color={Colors.primary} />
                  <Text style={[styles.groupHubBtnText, styles.groupHubBtnTextAlt]}>Add Group/People</Text>
                </TouchableOpacity>
              </View>
            </View>
          </>
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            {!conversationQuery.trim().length ? (
              <TouchableOpacity style={styles.emptyCtaBtn} onPress={() => router.push('/(client)/lawyers')}>
                <Text style={styles.emptyCtaText}>Find a Lawyer</Text>
              </TouchableOpacity>
            ) : null}
          </View>
        }
        renderItem={({ item }) => {
          ...
          const unreadLabel = Number(item.unread ?? 0) > 99 ? '99+' : String(item.unread ?? 0);
            ...
            <View style={{ flex: 1 }}>
              ...
              {isUnread ? (
                <View style={styles.newMessagePill}>
                  <Icon name="ellipse" size={7} color="#fff" />
                  <Text style={styles.newMessageText}>
                    {Number(item.unread ?? 0) > 1 ? `${item.unread} new messages` : 'New message'}
                  </Text>
                </View>
              ) : sentByMe ? (
      ...
      <Modal visible={showContactModal} transparent animationType="fade" onRequestClose={() => setShowContactModal(false)}>
        <View style={styles.contactModalOverlay}>
          ...
          <View style={styles.contactModalCard}>
            <Text style={styles.contactModalTitle}>Add Contact</Text>
            ...
            {contactQuery.trim().length > 0 && contactQuery.trim() !== debouncedContactQuery ? (
              <View style={styles.contactInlineState}>
                <ActivityIndicator size="small" color={Colors.primary} />
                <Text style={styles.contactInlineStateText}>Searching...</Text>
              </View>
            ) : null}
            ...
            {contactsLoading ? (
              ...
              <RNScrollView keyboardShouldPersistTaps="handled" style={styles.contactList}>
                {contactsError ? (
                  <View style={styles.contactErrorRow}>
                    <Icon name="warning-outline" size={15} color={Colors.error} />
                    <Text style={styles.contactErrorText}>{contactsError}</Text>
                    <TouchableOpacity style={styles.contactRetryBtn} onPress={() => void loadContactUsers()} disabled={contactsLoading}>
                      <Text style={styles.contactRetryText}>Retry</Text>
                    </TouchableOpacity>
                  </View>
                ) : null}

                {debouncedContactQuery.length > 0 && debouncedContactQuery.length < CONTACT_SEARCH_MIN_CHARS ? (
                  <Text style={styles.contactEmpty}>Type at least {CONTACT_SEARCH_MIN_CHARS} characters to search by name, email, or phone.</Text>
                ) : null}

                {groupedContacts.existing.length > 0 ? (
                  <>
                    <Text style={styles.contactSectionTitle}>Existing Conversations</Text>
                    {groupedContacts.existing.map((item) => {
                      const roleLabel = item.role ? item.role.toUpperCase() : 'USER';
                      const subtitle = item.phone || item.email || roleLabel;
                      const searchingByEmailOrPhone = debouncedContactQuery.length > 0;
                      return (
                        <TouchableOpacity
                          key={`existing-${item.id}`}
                          style={styles.contactRow}
                          onPress={() => openOrStartConversation(item)}
                          disabled={addingContactId === item.id}
                        >
                          ...
                          <View style={{ flex: 1 }}>
                            ...
                            <View style={styles.contactRatingRow}>
                              <Icon
                                name={item.rating && item.rating > 0 ? 'star' : 'star-outline'}
                                size={12}
                                color={item.rating && item.rating > 0 ? '#F4B400' : Colors.textLight}
                              />
                              <Text style={[styles.contactRatingText, (!item.rating || item.rating <= 0) && styles.contactRatingTextMuted]}>
                                {item.rating && item.rating > 0 ? Number(item.rating).toFixed(1) : 'No ratings yet'}
                                {item.rating && item.rating > 0 && Number(item.review_count || 0) > 0
                                  ? ` (${Number(item.review_count || 0)})`
                                  : ''}
                              </Text>
                            </View>
                            <Text style={styles.contactMeta} numberOfLines={1}>{subtitle}</Text>
                            {searchingByEmailOrPhone && item.email && item.phone ? (
                              <Text style={styles.contactMetaSmall}>{item.email}</Text>
                            ) : null}
                          </View>
                ...
                {groupedContacts.fresh.length > 0 ? (
                  <>
                    <Text style={styles.contactSectionTitle}>Start New Chat</Text>
                    {groupedContacts.fresh.map((item) => {
                      const roleLabel = item.role ? item.role.toUpperCase() : 'USER';
                      const subtitle = item.phone || item.email || roleLabel;
                      const searchingByEmailOrPhone = debouncedContactQuery.length > 0;
                      return (
                        ...
                        >
                          ...
                          {addingContactId === item.id ? (
                            ...
                            <View style={styles.contactTagNew}><Text style={styles.contactTagNewText}>Start chat</Text></View>
                          )}
                        </TouchableOpacity>
                      );
                    })}
                ...
                {debouncedContactQuery.length >= CONTACT_SEARCH_MIN_CHARS && !groupedContacts.existing.length && !groupedContacts.fresh.length ? (
                  <View style={styles.inviteCtaWrap}>
                    <Text style={styles.contactEmpty}>No contacts matched your search.</Text>
                    <Text style={styles.inviteCtaHint}>You can invite this contact to LexConnect instead.</Text>
                    <View style={styles.inviteCtaRow}>
                      <TouchableOpacity style={[styles.inviteBtn, !invitePhone && styles.inviteBtnDisabled]} onPress={inviteBySms} disabled={!invitePhone}>
                        <Icon name="chatbubble-ellipses-outline" size={15} color="#fff" />
                        <Text style={styles.inviteBtnText}>Invite by SMS</Text>
                      </TouchableOpacity>
                      <TouchableOpacity style={[styles.inviteBtnAlt, !inviteIsEmail && styles.inviteBtnDisabled]} onPress={inviteByEmail} disabled={!inviteIsEmail}>
                        <Icon name="mail-outline" size={15} color={inviteIsEmail ? Colors.primary : Colors.textLight} />
                        <Text style={[styles.inviteBtnAltText, !inviteIsEmail && styles.inviteBtnAltTextDisabled]}>Invite by Email</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                ) : null}
...
const styles = StyleSheet.create({
  ...
  groupHubTopRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  groupHubIconWrap: {
    width: 34,
    height: 34,
    borderRadius: 10,
  ...
  groupHubTitle: { color: Colors.text, fontSize: 15, fontWeight: '800' },
  groupHubSub: { color: Colors.textMuted, fontSize: 12, marginTop: 3 },
  groupHubUnreadBadge: {
    minWidth: 24,
    height: 24,
    borderRadius: 12,
  ...
  groupHubUnreadText: { color: '#fff', fontWeight: '800', fontSize: 11 },
  groupHubActions: { flexDirection: 'row', gap: 8, marginTop: 10 },
  groupHubBtn: {
    flex: 1,
    backgroundColor: Colors.primary,
    borderRadius: 10,
  ...
  groupHubBtnText: { color: '#fff', fontWeight: '700', fontSize: 12 },
  groupHubBtnTextAlt: { color: Colors.primary },
  errorCard: {
    marginBottom: 10,
    borderWidth: 1,
    borderColor: '#F2C7C7',
  ...
  emptyWrap: { flexGrow: 1, paddingHorizontal: 16, paddingBottom: 16 },
  convCard: {
    backgroundColor: '#fff',
    borderRadius: 18,
    borderWidth: 1,
  ...
  newMessageText: { color: '#fff', fontSize: 11, fontWeight: '800' },
  unreadBadge: {
    minWidth: 22,
    height: 22,
    borderRadius: 11,
  ...
  chatHeaderStatusRow: { marginTop: 2, flexDirection: 'row', alignItems: 'center', gap: 6 },
  chatHeaderDot: { width: 7, height: 7, borderRadius: 99, backgroundColor: '#94A3B8' },
  chatHeaderDotActive: { backgroundColor: '#22C55E' },
  chatHeaderDotBusy: { backgroundColor: '#FBBF24' },
  chatHeaderDotOffline: { backgroundColor: '#94A3B8' },
  chatHeaderStatusText: { color: 'rgba(255,255,255,0.78)', fontSize: 11, fontWeight: '700' },
  ...
  threadActionsPrimaryText: { color: '#fff', fontWeight: '800', fontSize: 14 },
  ...
  contactInlineState: { marginTop: 8, flexDirection: 'row', alignItems: 'center', gap: 8 },
  contactInlineStateText: { color: Colors.textMuted, fontSize: 12, fontWeight: '600' },
  ...
  contactSectionTitle: { color: Colors.textMuted, fontSize: 11, fontWeight: '800', letterSpacing: 0.5, marginBottom: 8, marginTop: 2 },
  contactErrorRow: {
    marginBottom: 10,
    borderWidth: 1,
    borderColor: '#F2C7C7',
  ...
  contactErrorText: { color: Colors.error, fontSize: 12, fontWeight: '700', flex: 1 },
  contactRetryBtn: {
    borderWidth: 1,
    borderColor: '#E7B5B8',
    borderRadius: 999,
  ...
  contactRetryText: { color: Colors.error, fontSize: 11, fontWeight: '800' },
  ...
  inviteCtaHint: { color: Colors.textMuted, textAlign: 'center', fontSize: 12, marginTop: -10, marginBottom: 10 },
  inviteCtaRow: { flexDirection: 'row', gap: 8 },
  inviteBtn: {
    flex: 1,
    backgroundColor: Colors.primary,
    borderRadius: 10,
  ...
  inviteBtnAltText: { color: Colors.primary, fontSize: 12, fontWeight: '800' },
  inviteBtnAltTextDisabled: { color: Colors.textLight },
  inviteBtnDisabled: { opacity: 0.48 },
  contactRow: {
    borderWidth: 1,
    borderColor: '#E7EDF7',
    borderRadius: 12,
  ...
  contactRatingRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 2 },
  contactRatingText: { color: Colors.text, fontSize: 11, fontWeight: '700' },
  contactRatingTextMuted: { color: Colors.textMuted, fontWeight: '600' },
  contactMeta: { color: Colors.textMuted, fontSize: 12, marginTop: 2 },
  contactMetaSmall: { color: Colors.textLight, fontSize: 11, marginTop: 1 },
  contactTagExisting: {
