...
import { groupApi, lawFirmApi } from '@/services/api';
...
export default function MessagesScreen() {
  ...
  const { triggerLawFirmUnreadRefresh } = useNotifications();
  ...
  useEffect(() => {
    ...
    navigation.setOptions({
      tabBarStyle: isConversationOpen
        ? { ...LAW_FIRM_TAB_BAR_STYLE, display: 'none' }
        : LAW_FIRM_TAB_BAR_STYLE,
    });

    return () => {
      tabBarHiddenRef.current = null;
      navigation.setOptions({ tabBarStyle: LAW_FIRM_TAB_BAR_STYLE });
    };
  ...
  const retryFailedMessage = useCallback(async (message: Message) => {
    ...
    try {
      if (message.local_attachment_uri && message.attachment_name) {
        ...
        const { data } = await lawFirmApi.sendMessageWithAttachment(selected.id, message.body || '', {
        ...
        const { data } = await lawFirmApi.sendMessage(selected.id, message.body || '');
        updateOptimisticMessage(message.local_id, {
          id: data?.id ?? Date.now(),
          body: data?.body ?? message.body,
          created_at: data?.created_at ?? new Date().toISOString(),
  ...
  const deleteMessage = useCallback(async (message: Message, mode: 'me' | 'everyone') => {
    ...
    try {
      await lawFirmApi.deleteMessage(message.id, mode);
      removeMessageLocally(message);
      loadConversations();
    } catch (error: any) {
  ...
  const loadMessages = useCallback(async (conv: Conversation) => {
    ...
    try {
      const { data } = await lawFirmApi.threadMessages(conv.id, { limit: 120 });
  ...
  const openConversation = useCallback((conv: Conversation) => {
    ...
    triggerLawFirmUnreadRefresh();
    loadMessages(conv);
  }, [loadMessages, prioritizeThreadLatest, resetScrollGuards, triggerLawFirmUnreadRefresh]);
  ...
  const openOrStartConversationFromContact = useCallback(async (contact: ContactUser) => {
    ...
    if (contact.role && contact.role !== 'client') {
      Alert.alert('Unavailable', 'Law firms can only start direct chats with client accounts.');
      return;
    }

    setAddingContactId(contact.id);
    try {
      await lawFirmApi.startConversation(contact.id);
      const { data } = await lawFirmApi.conversations();
  ...
  }, [existingConversationByUser, openConversation, triggerLawFirmUnreadRefresh]);
  ...
  const closeConversation = useCallback(() => {
    ...
    loadConversations();
    triggerLawFirmUnreadRefresh();
  }, [loadConversations, navigation, resetScrollGuards, triggerLawFirmUnreadRefresh]);
  ...
  async function sendMessage(text?: string) {
    ...
    setNewMsg('');
    try {
      const { data } = await lawFirmApi.sendMessage(selected.id, body);
    ...
    async function sendAttachment(uri: string, name: string, mimeType: string) {
      ...
      setNewMsg('');
      try {
        const { data } = await lawFirmApi.sendMessageWithAttachment(selected.id, body, { uri, name, type: mimeType });
        updateOptimisticMessage(localId, {
          id: data?.id ?? Date.now(),
          body: data?.body ?? body,
          created_at: data?.created_at ?? new Date().toISOString(),
      ...
      >
        ...
        >
          <View style={styles.headerTopRow}>
            <Text style={styles.headerEyebrow}>LAW FIRM SPACE</Text>
            <Text style={styles.headerTitle}>Messages</Text>
            <Text style={styles.headerSub}>Manage conversations with clients and keep updates in one thread.</Text>
          ...
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySub}>
              {conversationQuery.trim().length
                ...
                : 'Start by opening a consultation or messaging a client.'}
            </Text>
            {!conversationQuery.trim().length ? (
              <TouchableOpacity style={styles.emptyCtaBtn} onPress={() => router.push('/(lawfirm)/consultations')}>
...
const styles = StyleSheet.create({
  ...
  headerCard: {
    padding: 16,
    borderRadius: 18,
    backgroundColor: RoleColors.lawFirm.shellDark,
  },
  historyLoaderWrap: {
    paddingVertical: 10,
    alignItems: 'center',
  ...
  headerEyebrow: { color: RoleColors.lawFirm.accentSoft, fontSize: 11, fontWeight: '700', letterSpacing: 0.4 },
  headerTitle: { color: '#fff', fontSize: 24, fontWeight: '800', marginTop: 4 },
  headerSub: { color: 'rgba(255,255,255,0.82)', fontSize: 13, marginTop: 4, lineHeight: 18 },
  headerTopRow: { gap: 2 },
  ...
  overviewLabel: { color: RoleColors.lawFirm.accentSoft, fontSize: 11, fontWeight: '700', marginTop: 2 },
  newContactBtn: {
  ...
  threadActionsAvatarText: { color: RoleColors.lawFirm.shell, fontSize: 16, fontWeight: '900' },
